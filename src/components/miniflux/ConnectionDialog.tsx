...
export function ConnectionDialog({
  ...
  const form = useForm({
    defaultValues: {
      // The browser build defaults to this site, whose `/miniflux-api` proxy
      // needs no CORS headers on the Miniflux end. Typing another server is
      // allowed and goes direct — see `apiBaseFor()` in `@/lib/web/client`.
      // Desktop starts empty: `window.location.origin` there is the Tauri
      // shell's own URL, which is no one's Miniflux.
      serverUrl: isWeb ? window.location.origin : '',
      authToken: '',
      username: '',
      password: '',
              ...
              {(field) => (
                <Field>
                  ...
                  <FieldDescription id={`${field.name}-help`}>
                    {isWeb
                      ? _(
                          msg`Leave this site's own address to go through its proxy. Another server works only if it returns CORS headers for this site.`
                        )
