      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          ...
          {/*
            ...
            tappable.
          */}
          <div className="flex h-full w-full flex-col pt-[env(safe-area-inset-top,0px)] pr-[env(safe-area-inset-right,0px)] pb-[max(env(safe-area-inset-bottom,0px),var(--vv-offset-bottom,0px))] pl-[env(safe-area-inset-left,0px)]">
            {children}
          </div>
        </SheetContent>
      </Sheet>
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    ...
    >
      ...
      <span className="sr-only">{i18n._(msg`Toggle Sidebar`)}</span>
    </Button>
  );
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      aria-label={i18n._(msg`Toggle Sidebar`)}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={i18n._(msg`Toggle Sidebar`)}
      className={cn(
...
function SidebarGroupAction({
  ...
  return useRender({
    ...
    props: mergeProps<'button'>(
      {
        className: cn(
          'text-sidebar-foreground ring-sidebar-ring hover:bg-black/[0.06] dark:hover:bg-white/10 hover:text-sidebar-accent-foreground absolute top-3.5 right-3 w-5 rounded-md p-0 focus-visible:ring-2 [&>svg]:size-4 flex aspect-square items-center justify-center outline-hidden transition-transform [&>svg]:shrink-0 after:absolute after:-inset-2 md:after:hidden group-data-[collapsible=icon]:hidden',
          className
        ),
      },
      props
...
const sidebarMenuButtonVariants = cva(
  'hover:bg-black/[0.06] dark:hover:bg-white/10 hover:text-sidebar-accent-foreground active:bg-black/[0.06] dark:active:bg-white/10 active:text-sidebar-accent-foreground data-active:bg-black/[0.06] dark:data-active:bg-white/10 data-active:text-sidebar-accent-foreground data-open:hover:bg-black/[0.06] dark:data-open:hover:bg-white/10 data-open:hover:text-sidebar-accent-foreground gap-2 rounded-md py-1 px-2 text-left text-sm transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! data-active:font-medium peer/menu-button flex w-full items-center overflow-hidden outline-hidden group/menu-button disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        default:
          'hover:bg-black/[0.06] dark:hover:bg-white/10 hover:text-sidebar-accent-foreground',
        outline:
          'bg-background hover:bg-black/[0.06] dark:hover:bg-white/10 hover:text-sidebar-accent-foreground shadow-[0_0_0_1px_hsl(var(--sidebar-border))] hover:shadow-[0_0_0_1px_hsl(var(--sidebar-accent))]',
      },
      size: {
...
function SidebarMenuBadge({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'text-sidebar-foreground peer-hover/menu-button:text-sidebar-accent-foreground peer-data-active/menu-button:text-sidebar-accent-foreground pointer-events-none absolute top-1/2 right-1 flex h-5 min-w-5 -translate-y-1/2 rounded-md px-1 text-xs font-medium items-center justify-center tabular-nums select-none group-data-[collapsible=icon]:hidden',
        className
      )}
      {...props}
    />
